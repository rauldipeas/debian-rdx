#!/bin/bash
set -e
mkdir -p /tmp/debian-rdx/config/includes.chroot/usr/share/{backgrounds,gnome-background-properties}
cp -r assets/unsplash /tmp/debian-rdx/config/includes.chroot/usr/share/backgrounds/unsplash
cat <<EOF |tee /tmp/debian-rdx/config/includes.chroot/usr/share/gnome-background-properties/unsplash.xml>/dev/null
<?xml version="1.0"?>
